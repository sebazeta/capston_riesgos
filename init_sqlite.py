"""
Script de inicialización del Proyecto TITA con SQLite
Base de datos robusta que NO se corrompe

Uso: python init_sqlite.py
"""
import os
import sys
...
from services.database_service import (
    init_database, insert_rows, read_table, DB_PATH
)
...
def main():
    ...
    print("=" * 60)
    print("🏢 PROYECTO TITA - Inicialización SQLite")
    print("=" * 60)
    
    # Eliminar base de datos existente
    if os.path.exists(DB_PATH):
        os.unlink(DB_PATH)
        print(f"   🗑️ Eliminada BD anterior: {DB_PATH}")
    ...
    insert_rows("CRITERIOS_MAGERIT", get_criterios_magerit())
    print("   ✅ CRITERIOS_MAGERIT (15 registros)")
    
    insert_rows("CATALOGO_AMENAZAS_MAGERIT", get_amenazas_magerit())
    print("   ✅ CATALOGO_AMENAZAS_MAGERIT (10 registros)")
    
    insert_rows("CATALOGO_ISO27002_2022", get_controles_iso27002())
    print("   ✅ CATALOGO_ISO27002_2022 (5 registros)")
    
    # Bancos de preguntas
    print("📝 Creando bancos de preguntas...")
    
    preguntas_fisicas = get_banco_preguntas_fisicas()
    insert_rows("BANCO_PREGUNTAS_FISICAS", preguntas_fisicas)
    print(f"   ✅ BANCO_PREGUNTAS_FISICAS ({len(preguntas_fisicas)} preguntas)")
    
    preguntas_virtuales = get_banco_preguntas_virtuales()
    insert_rows("BANCO_PREGUNTAS_VIRTUALES", preguntas_virtuales)
    print(f"   ✅ BANCO_PREGUNTAS_VIRTUALES ({len(preguntas_virtuales)} preguntas)")
    
    # Verificar
    print("\n📊 Verificación:")
    for table in ["CRITERIOS_MAGERIT", "CATALOGO_AMENAZAS_MAGERIT", "CATALOGO_ISO27002_2022",
    ...
    print(f"   💪 SQLite NO se corrompe con escrituras concurrentes")
